...
void *producer (void *parg)
{
    ...
    while (1) {
        ...
        while (fifo->full && *total_produced != WORK_MAX) {
            printf ("prod %d:  FULL.\n", my_tid);
            // wait until fifo is not full -> dont spin 
        ...
        // since something has been added, the queue cannot be empty 
...
void *consumer (void *carg)
{
    ...
    while (1) {
        ...
        while (fifo->empty && *total_consumed != WORK_MAX) {
            printf ("con %d:   EMPTY.\n", my_tid);
            // wait until fifo is not empty -> dont spin
        ...
        // since something has been removed, the queue cannot be full
...
int main (int argc, char *argv[])
{
    ...
    for (i=0; i<pros; i++){ 
        ...
        memset(thread_args, 0, sizeof(pcdata));
        /*
