...
struct Storage {
  ...
  // FIXED: increased the size of sentence from 6 to 7
...
void echoohce(char** strs) {
  ...
  for (iter = strs; *iter != NULL; ++iter)
    printf("%s ", *iter);
    // FIXED: dereferenced iter pointer when passing it to "printf()"
  ...
  for (; *iter != *stop_beginning; --iter) 
    // FIXED: changed loop end condition to the beginning of iter
    printf("%s ", *iter);
...
int main(int argc, char** argv) {
  ...
  // FIXED: initialized "num_bugs_on_mars" to 0
  ...
  // FIXED: free memory of "useless" info before reassigning it
  ...
  // FIXED: pass the 6th element of bug info instead of the 2nd because the
  // 2nd element has already been freed from memory 
  ...
  // FIXED: bug_info.useless is part of a struct and is implicitly freed 
