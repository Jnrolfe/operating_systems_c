...
typedef struct {
    ...
    pthread_t      thread;       /* Thread structure for this
                                    philosopher */
    int            waited;       /* How many times the philosopher has
                                    been awakened but the chopsticks are not
                                    available */
} philosopher;
...
int my_chopsticks_free(philosopher* p)
{   
    int l_chop = *(left_chop_available(p));
    int r_chop = *(right_chop_available(p));
...
void mark_my_chopsticks_taken(philosopher* p)
{
    available_chopsticks[p->id] = 0;
    available_chopsticks[(p->id == 0 ? NUM_CHOPS-1 : (p->id)-1)] = 0;
    return;
}

void mark_my_chopsticks_free(philosopher* p)
{
    available_chopsticks[p->id] = 1;
    available_chopsticks[(p->id == 0 ? NUM_CHOPS-1 : (p->id)-1)] = 1;
    return;
}
...
static void *dp_thread(void *arg)
{
    ...
    while (!Stop) {
        ...
        // signal those who might care
        /*
        pthread_cond_broadcast(&(right_phil(me)->can_eat));
        pthread_cond_broadcast(&(left_phil(me)->can_eat));
        */

        // only signal to wake if both chopsticks are free
        if(!(Smart_signaling))
        {
            Smart_signaling = 1;
        }
        philosopher* r_p = right_phil(me);
        philosopher* l_p = left_phil(me);
        if(my_chopsticks_free(r_p))
        {
            pthread_cond_broadcast(&(r_p->can_eat));
        }
        else if(my_chopsticks_free(l_p))
        {
            pthread_cond_broadcast(&(l_p->can_eat));
        } 
...
out:
    ...
    int main(int argc, char **argv)
    {
        ...
        {
            sprintf(buf, 
                    "* Only signaled to wake philosophers if both of their"
                    " chopsticks were marked available\n"
                   );
            printf("%s\n", buf);
        }

        /* Print number of times each philosopher waited */
        printf("Number of times waited:\n");
        for(i = 0; i < NUM_PHILS; i++)
        { 
            sprintf(buf, "%d", Diners[i].waited);
            printf("p%d= %s ", i, buf);
        }
        printf("\n\n"); // correct spacing for next print out
