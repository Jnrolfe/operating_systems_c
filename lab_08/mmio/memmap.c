...
int main (int argc, char *argv[])
{
     ...
     */
    if(fstat(fdin, &statbuf) < 0) // is zero if correct
    {
        err_sys("Error getting size\n");
    }

    /* 
     * 2. go to the location corresponding to the last byte 
     */
    if(lseek(fdout, statbuf.st_size-1, SEEK_SET) < 0) // is zero if correct
    {
        err_sys("Error getting the location\n");
    }

    /* 
     * 3. write a dummy byte at the last location 
     */
    if(write(fdout, "", 1) < 0) // is zero if correct
    {
        err_sys("Not able to write dummy byte\n");
    }
    ...
    {
        err_sys("Error mmapping the input file\n");
    }

    /* 
     * 5. mmap the output file 
     */
    if((dst = mmap(0, statbuf.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fdout, 0)) < 0)
    {
        err_sys("Error mmapping the output file\n");
    }
     ...
     */
    // copy the memory now that dst and src have been found
